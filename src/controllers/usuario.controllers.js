...
// Crear usuario con validación de clave autorización
async function createUsuario(req, res) {
  const dataUsuarios = req.body;

  // Definí la clave secreta para permitir registro
  const CLAVE_AUTORIZACION = "miClaveSuperSegura"; // Cambiá esta clave por la que uses

  try {
    if (dataUsuarios.claveAutorizacion !== CLAVE_AUTORIZACION) {
      return res.status(403).json({
        ok: false,
        message: "Clave de autorización incorrecta. No puedes registrarte.",
      });
    }

    // Verificar si ya existe el correo
    const usuarioExistente = await Usuario.findOne({
      where: { correo: dataUsuarios.correo },
    });
    if (usuarioExistente) {
...
async function loginUsuario(req, res) {
  ...
  try {
    ...
    if (!usuario) {
      return res
        .status(401)
        .json({ ok: false, message: "Correo no encontrado" });
    }
